...
RSpec.describe Keycloak::Client do
  ...
  it 'obtain a new refresh token when the refresh token is expired' do
    ...
    expect($client.access_token_valid?).to eq(false)
    expect($client.refresh_token_valid?).to eq(false)
    $client.find_realm($client.realm)
    expect($client.access_token_valid?).to eq(true)
    expect($client.refresh_token_valid?).to eq(true)
